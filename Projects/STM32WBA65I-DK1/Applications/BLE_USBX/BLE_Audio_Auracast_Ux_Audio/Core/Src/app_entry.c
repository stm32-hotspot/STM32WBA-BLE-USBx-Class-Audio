...
static void DrawStatusLCDTask(void);
static void DrawStatusLCD(void);
#endif /* CFG_LCD_SUPPORTED */
...
#if (CFG_LCD_SUPPORTED == 1)
  LCD_Init();
  UTIL_SEQ_RegTask(1U << CFG_TASK_DRAW_STATE_LCD_ID, UTIL_SEQ_RFU, DrawStatusLCDTask);
#endif /* CFG_LCD_SUPPORTED */
...
static void LCD_Init( void )
{
  ...
  LCD_DrawBitmapArray(6 , 32, 32, 32, &auracast_logo[0]);
  LCD_DrawBitmapArray(42 , 40, 16, 16, &nok_icon[0]);
  LCD_DrawBitmapArray(70 , 32, 32, 32, &usb_logo[0]);
  LCD_DrawBitmapArray(106 , 40, 16, 16, &nok_icon[0]);
...
static void DrawStatusLCD(void)
{
  ...
  switch (gAuracast_state)
  {
    case AURACAST_STATE_OFF:
      /* Draw NOK icon */
      auracast_state_logo = &nok_icon[0];
      break;

    case AURACAST_STATE_CONFIGURED:
      /* Draw OK icon */
      auracast_state_logo = &ok_icon[0];
      break;

    case AURACAST_STATE_STREAMING:
      /* Draw Speaker icon */
      auracast_state_logo = &speaker_icon[0];
      break;
  }
  ...
  if (auracast_state_logo != 0)
  {
    LCD_DrawBitmapArray(42 , 40, 16, 16, auracast_state_logo);
  }
  if (usb_state_logo != 0)
  {
    LCD_DrawBitmapArray(106 , 40, 16, 16, usb_state_logo);
  }
}
#endif  /* CFG_LCD_SUPPORTED */
